        ...
        <div className='network-persons'>
          {
            users?.map((user,i) => {
              let exists = notifications.some(notification => {
                return notification.user_id1 == user._id || notification.user_id2 == user._id
              })
              let card = <div className='one-person' key={i}>
                ...
                {
                  exists ? <button className='pending-friend'>Pending</button> : <button className='add-friend' id={user._id} onClick={handleFriendRequest}>Add Friend</button>
                }
              </div>
              return card
            })
